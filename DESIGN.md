...
which is great for handling transactions. The schema is designed to optimize performance, with appropriate indexing to speed up queries, especially for the offers eligibility endpoint. I used int to store money values as suggested, but it's also worth thinking about numeric type, as it supports more decimals and it'd be better if multiple currencies are 
a possibility.
...
This project uses go-jet to build the queries using idiomatic Go code. It has better performance than an ORM and I think it's nice to use.
## Eligibility Logic
...
The query applies all dates, MCCs and merchant filters using JOINS and subqueries.
## Considerations
